...
Logging in takes two steps: username + password, then a 6-digit verification code (`POST /api/staff/login` then `/api/staff/login/verify-code`, backed by the `staff_login_codes` table). With no email on file for the account (or real email delivery not set up — see **Setting up real email delivery** below), the code comes back in the same response and is shown on screen, the same "simulated" delivery every other code in this app falls back to. The moment a staff member adds their own email on the portal's **Employees** tab ("My email") *and* real delivery is configured, the code goes there instead — turning this from a genuine second *step* into a genuine second *factor*, with no other changes needed.
