...
- **A permanent audit log** (`staff_audit_log`, visible under the portal's **Audit log** tab, owner-only) — every cash-out marked paid or rejected, and every new staff account created, is recorded with exactly who did it and when. Nothing in the app ever updates or deletes a log entry, so it's a trustworthy record an owner can check, not something an employee could tidy up after themselves.
